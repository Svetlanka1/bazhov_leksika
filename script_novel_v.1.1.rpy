...
screen word_choice_screen():
        frame:
            ...
            vbox:
                spacing 25
                label "Какое слово пропало? Оно и есть - та самая драгоценность из шкатулки Настасьи":
                    text_size 34
                    xalign 0.5
                ...
                grid 2 2:
                    ...
                    textbutton "срядилось" action Return("срядилось")  
                    textbutton "выпросталось" action Return("выпросталось")
                    textbutton "скыркалось" action Return("скыркалось")
                    textbutton "причтелось" action Return("причтелось")  # правильный ответ
...
label choice_1:
    ...
    ba "{b}Щёгарь{/b} - это горный мастер. Работал Степан в горах. Малахит добывал."
...
label sunduk:
    ...
    n "А вот и мужнина шкатулка с богатствами!"
    n "Ой, а одно украшение пропало, кажись..."
    n "Поможете найти?"
    jump game_1

label game_1:
    call screen word_choice_screen()

    # Проверяем, КАКОЕ слово вернулось
    $ chosen_word = _return

    if chosen_word == "причтелось":
        n "Ой, неужели нашли? Ну, слава Богу!"
        n "Слова наши - действительно драгоценности!"
        n "А {b}причтелось{/b} — значит, пришлось."
        $ znanie += 1
        jump znakomstvo

    else:
        # Объясняем каждое неправильное слово
        if chosen_word == "срядилось":
            n "Нет, другая драгоценность потерялась."
            n "{b}Срядиться{/b} - это не то. Это ж, значит, договориться."
            n "О покупке какой или ещё о чем."
            n "Так что не то это."


        elif chosen_word == "выпросталось":
            n "Нет, не эта драгоценность потерялась."
            n "{b}Выпростать{/b} говорят, когда что-то вытащить удалось."
            n "Например, ежели расчёска в волосах запутается, то её оттуда выпростать нужно."
            n "Так что не то это."

        elif chosen_word == "скыркалось":
            n "Нет, не эта драгоценность потерялась."
            n "{b}Скыркаться{/b} - это в земле ковыряться, рыться."
            n "Отец Танюшки ведь с малых лет этим и занимался."
            n "Так что не то это."

        $ error += 1
        n "Ну-ка, поищите драгоценное словечко получше."
        jump game_1
